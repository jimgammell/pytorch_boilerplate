...
class CosineDecayLRSched(optim.lr_scheduler.LambdaLR):
    def __init__(self,
        optimizer: optim.Optimizer, total_steps: int, warmup_steps: int = 0, const_steps: int = 0,
        warmup_lr_prop: float = 0.0, final_lr_prop: float = 1e-1
    ):
        ...
        self.scheduler = np.concatenate([
            np.linspace(warmup_lr_prop, 1, warmup_steps),
            np.ones(const_steps),
