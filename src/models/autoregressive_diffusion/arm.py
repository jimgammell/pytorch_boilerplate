...
class ARM(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.position_embedding = nn.Embedding(self.config.vocab_size, self.config.embedding_dim)
    ...
    def _load_pretrained_model(self, path: str):
        ...
        for k, v in hf_state_dict.items():
            ...
            k.replace('wte', 'token_embedding')
            k.replace('wpe', 'position_embedding')
            k.replace('h', 'trunk.transformer_blocks')
            k.replace('ln_1', 'attn_norm')
            k.replace('ln_2', 'fnn_norm')
            k.replace('mlp', 'fnn')
            k.replace('attn.c_attn', 'attn.to_qkv')
            k.replace('attn.c_proj', 'attn.to_out')
            k.replace('fnn.c_fc', 'fnn.dense_1')
            k.replace('fnn.c_proj', 'fnn.dense_2')
            k.replace('ln_f', 'head.norm')
            k.replace('lm_head', 'head.dense')
            state_dict[k] = v
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        position_tokens = torch.arange(0, sequence_length, dtype=torch.long, device=device)
    ...
    def autoregressive_sample(self, context: torch.Tensor, max_new_tokens: int, temperature=1.0, top_k=None) -> torch.Tensor:
